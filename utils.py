...
dictionary = Dictionary.load('/root/anaconda3/lib/python3.8/site-packages/unimol_tools/weights/mol.dict.txt')
...
def decorate_torch_batch(batch):
    """
    ...
    if isinstance(net_input, dict):
        net_input, net_target = {
            k: v for k, v in net_input.items()}, net_target
    else:
        net_input, net_target = {'net_input': net_input}, net_target
    net_target = None
    return net_input, net_target

def mol_batch_collate_fn(samples):
    """
    ...
    for k in samples[0].keys():
        if k == 'src_coord':
            ...
            v = pad_2d([torch.tensor(s[k]).long() for s in samples], pad_idx=padding_idx)
        elif k == 'src_distance':
            v = pad_2d([torch.tensor(s[k]).float() for s in samples], pad_idx=0.0)
        elif k == 'src_tokens':
            v = pad_1d_tokens([torch.tensor(s[k]).long() for s in samples], pad_idx=padding_idx)
        batch[k] = v
    try:
